...
docker build --build-arg BUILD_FROM=homeassistant/amd64-base:3.10 \
  --build-arg BUILD_ARCH=amd64 ./frp-client
```

## Version bumping & releasing (frp-client)

The add-on versions itself independently (`1.0.0`+ in `frp-client/config.yaml`); it does **not** track or bundle any frp version. `build.json` carries no frp argument — **no frpc is baked into the image**; the web UI downloads whichever release the user picks into `/data` on first use.

Releases are git tags in the format `<slug>-v<version>` (e.g. `frp_client_zdy-v1.1.0`); HA's add-on store offers each such tag as a selectable install version. A tag must point at a commit whose `config.yaml` version matches the tag.

## frp-client architecture

Single-stage, all runtime (`run.sh`, the container CMD): starts a built-in web config editor — two busybox `nc -lk -p 8080 -e /www/handler.sh` listeners, one on `127.0.0.1` and one on the `hassio` bridge gateway IP (discovered via `ip addr show hassio`, falling back to `172.30.32.1`) — exposed via HA ingress (`ingress: true` / `ingress_port: 8080` in `config.yaml`; assets in `frp-client/www/`, installed to `/www`). The base image's busybox has **no httpd applet**; `nc -e` does the job. The editor page POSTs the raw TOML text (no URL decoding needed); `handler.sh` writes `/data/frpc.toml` (the runtime config). The page has separate 保存 (save only) and 启动/重启 (save + apply) buttons; `handler.sh` communicates commands to `run.sh` via `/tmp/frpc.cmd` (`start`/`restart`). The main loop in `run.sh` consumes commands, auto-starts frpc at add-on boot when `/data/frpc.toml` exists, and a watchdog re-spawns frpc ~5s after an unexpected death. **There are no add-on options/schema at all** — everything (config *and* frp version) is managed through the web editor.

**frp version management** (web page「frp版本」): `handler.sh` exposes `/cgi-bin/frp/installed|available|download|select`. Downloads run detached from the HTTP connection (`nohup /www/dl.sh <ver> <mirror-prefix> &`) so page polling via `/cgi-bin/status` (reading `/tmp/frp.download.state`) tracks progress. `dl.sh` maps `uname -m` → Go arch (aarch64→arm64, x86_64→amd64, armv7l/armv6l→arm, i686/i386→386), downloads `frp_<ver>_linux_<arch>.tar.gz` from GitHub releases (mirror = plain URL-prefix concatenation, falls back to direct if the mirror fails), validates with `frpc -v`, and stores it in `/data/frp-versions/<ver>/frpc`. **Only the currently active version is kept**: every successful switch (download-and-use or select) deletes the other downloaded versions. The active version lives in `/data/frp-version`; `run.sh` re-resolves it in `resolve_frpc()` on every start — if no version is installed, starting frpc just logs a warning (no crash, no watchdog loop). First-use flow: open web page → pick & download a version → paste TOML → 启动.
