...
Key design point: configuration happens entirely through the HA add-on options page. **The add-on options UI renders `str` fields as single-line inputs and silently drops newlines on paste** — that's why `frpc_config` supports literal `\n` separators (auto-converted by `run.sh`) and there is a second, preferred option `frpc_config_base64` (whole TOML base64-encoded; takes precedence when set). `run.sh` guards every `bashio::config` call with `|| true` (see bashio gotcha above), writes the result to `/tmp/frpc.toml`, and logs the line count. There is no `/share` mapping; the only persistent directory is `/data` (for cert files etc. referenced in the user's TOML). The repo's `frp-client/frpc.toml` is only an example (installed in the image as `/frpc.toml.example`); `frps.toml` is server-side reference only and unused at runtime.
