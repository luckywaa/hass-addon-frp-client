...
Key design point: **the web editor is the intended config path** (direct multi-line paste). The add-on options page is only a fallback seed because **the HA options UI renders `str` fields as single-line inputs and silently drops newlines on paste**. All `bashio::config` calls must stay wrapped in `|| true` (see bashio gotcha above). The web server binds to loopback only — the add-on uses `host_network`, so a wildcard bind would expose the unauthenticated editor on the LAN. `/data` is the only persistent directory (config + any cert files referenced in the TOML). The repo's `frp-client/frpc.toml` is only an example (installed in the image as `/frpc.toml.example`); `frps.toml` is server-side reference only and unused at runtime.
